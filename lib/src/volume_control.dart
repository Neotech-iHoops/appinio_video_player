...
class _VolumeControlsState extends State<VolumeControls> {
  ...
  void onPanUpdate(DragUpdateDetails dragUpdateDetails) {
    ...
    if (dragUpdateDetails.delta.dy > 0) {
      widget.customVideoPlayerController.videoPlayerController.setVolume(volume - factor);
      setState(() {
        _progress = ((volume - factor) < 0 ? 0 : (volume - factor)) * (height ?? 0);
      });
    } else if (dragUpdateDetails.delta.dy < 0) {
      widget.customVideoPlayerController.videoPlayerController.setVolume(volume + factor);
      setState(() {
        _progress = ((volume + factor) > 1 ? 1 : (volume + factor)) * (height ?? 0);
      });
    }
  }
}
